...
# scm/config/network/ethernet_interface.py
...
class EthernetInterface(BaseObject):
    ...
    def max_limit(self) -> int:
        ...
        Returns:
            int: Current maximum limit value.
        """
    ...
    def create(
        ...
        Args:
            data: Dictionary containing the Ethernet Interface configuration

        Returns:
            EthernetInterfaceResponseModel: The created Ethernet Interface object

        """
        # Use the dictionary "data" to pass into Pydantic and return a modeled object
        ethernet_interface = EthernetInterfaceCreateModel(**data)

        # Convert back to a Python dictionary, removing any unset fields and using aliases
        payload = ethernet_interface.model_dump(
            exclude_unset=True,
            by_alias=True,
        )
    ...
    def get(
        ...
        Args:
            object_id: The ID of the Ethernet Interface to retrieve

        Returns:
            EthernetInterfaceResponseModel: The retrieved Ethernet Interface object
    ...
    def update(
        self,
        interface: EthernetInterfaceUpdateModel,
    ) -> EthernetInterfaceResponseModel:
        """Update an existing Ethernet Interface object.

        Args:
            interface: EthernetInterfaceUpdateModel instance containing the update data

        Returns:
            EthernetInterfaceResponseModel: The updated Ethernet Interface object
    ...
    def _build_container_params(
        ...
        Returns:
            dict: Dictionary containing non-None container parameters
        """
        return {
            k: v
    ...
    def list(
        ...
        Returns:
            List[EthernetInterfaceResponseModel]: A list of Ethernet Interface objects

        Raises:
            MissingQueryParameterError: If folder is empty string
            InvalidObjectError: If container parameters are invalid
        ...
        while True:
            ...
            if not isinstance(response["data"], list):
                raise InvalidObjectError(
                    ...
                    http_status_code=500,
                    details={
                        "error": "'data' field is not a list",
                        "response": response,
                    },
                )
            ...
            data = response["data"]
            object_instances = [
                EthernetInterfaceResponseModel(**item) for item in data
            ]
        ...
        # If exact_match is True, filter out objects that don't match exactly
        if exact_match:
            all_objects = [
                each
    ...
    def fetch(
        ...
        Args:
            name: The name of the Ethernet Interface to fetch
            folder: The folder in which the resource is defined
            snippet: The snippet in which the resource is defined
            device: The device in which the resource is defined

        Returns:
            EthernetInterfaceResponseModel: The fetched Ethernet Interface object

        Raises:
            MissingQueryParameterError: If name or folder is empty
            InvalidObjectError: If container parameters are invalid or object not found
        ...
        if (
            ...
            raise InvalidObjectError(
                message=f"Object with name '{name}' not found",
                error_code="E003",
                http_status_code=404,
                details={"error": "Object not found in response"},
            )
